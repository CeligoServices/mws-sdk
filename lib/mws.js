var https = require('https'), qs = require("querystring"), crypto = require('crypto'), xml2js = require('xml2js');
 ...
 * 
 * @param {String}
 *            accessKeyId Id for your secret Access Key (required)
 * @param {String}
 *            secretAccessKey Secret Access Key provided by Amazon (required)
 * @param {String}
 *            merchantId Aka SellerId, provided by Amazon (required)
 * @param {Object}
 *            options Additional configuration options for this instance
 */
 ...
 * request defined in one of the submodules. However, you can use call()
 * manually when a lower level of control is necessary (custom or new requests,
 * for example).
 * 
 * @param {Object}
 *            api Settings object unique to each API submodule
 * @param {String}
 *            action Api `Action`, such as GetServiceStatus or GetOrder
 * @param {Object}
 *            query Any parameters belonging to the current action
 * @param {Function}
 *            callback Callback function to send any results recieved
 */
AmazonMwsClient.prototype.call = function(api, action, query, callback) {
	...
	if (api.upload) {
		var body = query._BODY_, bformat = query._FORMAT_;
		delete query._BODY_;
	...
	var req = this.conn.request(options, function(res) {
		...
		res.addListener('end', function() {
			...
			if (data.slice(0, 5) == '<?xml' || data.indexOf('xmlns=') != -1)
 ...
 * 
 * @param {String}
 *            path Path of API call (used to build the string to sign)
 * @param {Object}
 *            query Any non-signature parameters that will be sent
 * @return {Object} Finalized object used to build query string of request
 */
AmazonMwsClient.prototype.sign = function(path, query) {
	var keys = [], sorted = {}, hash = crypto.createHmac("sha256",
	...
	var stringToSign = [ "POST", this.host, path, qs.stringify(sorted) ]
			.join("\n");

	// An RFC (cannot remember which one) requires these characters also be
	// changed:
 ...
 * 
 * @param {Object}
 *            request An instance of AmazonMwsRequest with params, etc.
 * @param {Function}
 *            callback Callback function used to process results/errors
 */
...
/**
 * Constructor for general MWS request objects, wrapped by api submodules to
 * keep things DRY, yet familiar despite whichever api is being implemented.
 * 
 * @param {Object}
 *            options Settings to apply to new request instance.
 */
 ...
 * 
 * @param {String}
 *            param Key of parameter (not ALWAYS the same as the param name!)
 * @param {Mixed}
...
AmazonMwsRequest.prototype.set = function(param, value) {
	var p = this.params[param], v = p.value = {};
...
AmazonMwsRequest.prototype.query = function() {
	...
	for ( var param in this.params) {
		var value = this.params[param].value, name = this.params[param].name, complex = (this.params[param].type === 'Complex'), required = this.params[param].required;
...
/**
 * Contructor for objects used to represent enumeration states. Useful when you
 * need to make programmatic updates to an enumerated data type or wish to
 * encapsulate enum states in a handy, re-usable variable.
 * 
 * @param {Array}
 *            choices An array of any possible values (choices)
 */
...
/**
 * Complex List helper object. Once initialized, you should set an add(args)
 * method which pushes a new complex object to members.
 * 
 * @param {String}
 *            name Name of Complex Type (including .member or subtype)
 */
 ...
 * 
 * @param {Object}
 *            query Query object to append to
 * @return {Object} query
 */
